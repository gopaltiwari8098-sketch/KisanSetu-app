...
function generateToken(farmerId) {
  return jwt.sign({ farmerId }, process.env.JWT_SECRET, { expiresIn: '7d' });
}
...
async function signup(req, res) {
  try {
    ...
    try {
      ...
      console.error('Verification email bhejne mein error:', emailErr.message);
    }
...
async function verifyEmail(req, res) {
  try {
    ...
    if (!token) {
      return res.status(400).json({ message: 'Verification token missing hai' });
    }

    const farmer = await Farmer.verifyByToken(token);
    if (!farmer) {
      return res.status(400).json({ message: 'Token invalid ya already use ho gaya hai' });
    }
...
async function getProfile(req, res) {
  try {
    ...
    if (!farmer) {
      return res.status(404).json({ message: 'Farmer nahi mila' });
    }
    res.json({
      fullName: farmer.full_name,
      email: farmer.email,
      phone: farmer.phone,
      state: farmer.state,
      memberSince: new Date(farmer.created_at).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })
    });
  } catch (err) {
    console.error('Get profile error:', err.message);
    res.status(500).json({ message: 'Profile fetch karne mein error' });
  }
}
